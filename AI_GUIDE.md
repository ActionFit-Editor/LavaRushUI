...
The package provides a complete authored neutral UGUI prefab, ActionFit-owned placeholder sprites, a replaceable theme asset, and a standalone composition root for ActionFit Lava Rush. The generated view is retained only as a missing-or-broken-prefab fallback and uses Unity built-in resources and solid UGUI geometry.
...
- `Runtime/Prefabs/LavaRushPresentation.prefab` is the canonical presentation. `Runtime/Prefabs/LavaRushDemo.prefab` and the CatDetective sample reference it instead of duplicating its UI hierarchy.
...
- The package includes ActionFit-authored neutral PNG sprites, an authored presentation/demo prefab, and a neutral theme asset. They are the editable package defaults, not copies of Cat Merge Cafe assets.
