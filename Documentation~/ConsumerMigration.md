# Lava Rush 0.2 Consumer Migration
...
| `com.actionfit.referencebinding` | `0.2.1` | Serialized reference validation |
| `com.actionfit.ui.foundation` | `2.0.5` | Global UGUI wrappers used by the original prefabs |
| `com.actionfit.ui.popup` | `0.1.1` | One whole-flow popup queue owner |
| `com.actionfit.lava-rush.ui` | `0.2.0` | Canonical direct controllers, 14 prefab roles, and 56 original PNGs |
| `com.actionfit.cat.app` | `0.2.0` | Cat product composition and explicit three-key Addressables registration |
...
`com.actionfit.lava-rush.theme.catmerge@0.2.0` is an optional presentation preset. No final Cat Runtime assembly depends on it, so it is not part of the mandatory installer profile.
...
3. After separate publication approval has produced every immutable tag, install `https://github.com/ActionFit-Editor/LavaRushInstaller.git#0.2.0`. Let Custom Package Manager complete package registration, durable ownership reload, and installer self-removal.
...
8. Install `com.actionfit.lava-rush.theme.catmerge@0.2.0` only when the optional preset is wanted.
...
Before package publication, rollback is a source-control revert to the pre-migration project snapshot; the `0.2.0` Git URLs are not installable evidence until their immutable tags exist.
