...
3. Explain that `LavaRushEngine` remains authoritative and that the UI package owns only immutable view mapping, action routing, the canonical nested `Runtime/Prefabs/Main/UI_LavaRush.prefab`, additive copies of all 14 original prefab roles and 56 original PNGs, required visual dependencies, replaceable localization/audio/profile/view-host services, and the standalone demo. Generated UI is diagnostic recovery only.
