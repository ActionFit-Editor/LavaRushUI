...
```json
{
  "dependencies": {
    ...
    "com.actionfit.ui.popup": "https://github.com/ActionFit-Editor/UI_Popup.git#0.1.2",
    "com.actionfit.lava-rush.ui": "https://github.com/ActionFit-Editor/LavaRushUI.git#0.2.4",
...
패키지는 원본 `UI_Text`/`UI_Button` 직렬화와 공용 `ScalePulse` 동작, 직렬화된 버튼 로컬라이징, 전체 이벤트의 popup queue slot과 직접 활성화 화면의 배경 물리 입력 차단 수명을 유지하기 위해 `com.actionfit.fonts.maplestory@1.0.0`, `com.actionfit.referencebinding@0.2.2`, `com.actionfit.ui.foundation@2.0.5`, `com.actionfit.ui.popup@0.1.2`, `com.unity.localization@1.5.5`, `com.unity.ugui@2.0.0`, `com.unity.modules.animation@1.0.0`을 선언합니다. 원본 프리팹의 UIEffect, UIParticle, SoftMask, UILighting 컴포넌트는 위의 정확한 top-level Git dependency를 요구하며 정상 설치 경로인 Lava Rush Installer가 모두 설치합니다. 세부 리비전과 역할은 `Documentation~/ExternalVisualDependencies.md`에 있습니다.
...
`0.2.2`는 `LavaRushActionTarget`이 prefab의 유효한 `UI_Text` 또는 `LocalizeStringEvent` 번역을 버튼 문구 권한으로 유지하도록 수정합니다. model의 영어 문구는 번역 결과가 없을 때만 fallback으로 사용합니다. 기존 번역 key, locale asset, prefab binding과 시각 baseline은 변경하지 않습니다. 이 patch는 아직 `com.actionfit.lava-rush.installer@0.2.0` 후보 graph에 포함되지 않았으며, installer 갱신과 publish는 별도 승인 작업입니다.

`0.2.3`은 `com.actionfit.ui.popup@0.1.2`를 사용해 직접 활성화되는 `UI_LavaRush` 화면 흐름 전체에 하나의 배경 물리 입력 차단 핸들을 유지합니다. 난이도·튜토리얼·매치·결과·종료 화면 전환에서는 재취득하지 않고, `HideAll`, disable, destroy에서 멱등 해제합니다. 팝업 버튼, 14개 프리팹과 56개 이미지, GUID·직렬화, 엔진·보상 동작은 변경하지 않습니다. 이 patch 역시 installer 갱신과 publish가 별도 승인 사항입니다.

`0.2.4`는 `com.actionfit.referencebinding@0.2.2`의 package-owned Editor pump를 사용하고 enqueue 전용 consumer `OnValidate`를 제거합니다. `0.2.3`의 배경 물리 입력 차단 수명과 prefab, 직렬화 참조, GUID, fileID, Addressable identity, 엔진·보상 동작은 변경하지 않습니다.
